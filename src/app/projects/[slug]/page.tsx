...
const PROJECT_DETAIL_LINK_ICONS = {
  github: (
    <svg className="w-5 h-5 transition-transform duration-300 ease-soft group-hover:scale-110" fill="currentColor" viewBox="0 0 24 24">
  ...
  demo: (
    <svg className="w-5 h-5 transition-transform duration-300 ease-soft group-hover:-translate-y-[1px] group-hover:translate-x-[1px]" fill="none" stroke="currentColor" viewBox="0 0 24 24">
      <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M10 6H6a2 2 0 00-2 2v10a2 2 0 002 2h10a2 2 0 002-2v-4M14 4h6m0 0v6m0-6L10 14" />
    </svg>
  ),
  figjam: (
    <svg className="w-5 h-5 transition-transform duration-300 ease-soft group-hover:scale-110" fill="none" stroke="currentColor" viewBox="0 0 24 24">
...
export async function generateMetadata({ params }: ProjectPageProps): Promise<Metadata> {
  ...
  return getProjectPageMetadata({ frontmatter: project.frontmatter, slug }) as Metadata
}
...
export default async function ProjectPage({ params }: ProjectPageProps) {
  ...
  const imageUrl = resolveProjectImageUrl(frontmatter.image)
  const displayTitle = getProjectDetailDisplayTitle(frontmatter)
  const formattedDate = formatProjectDate(frontmatter.date)
  const breadcrumb = getProjectDetailBreadcrumb({ isPlayground })
  const projectLinks = getProjectDetailLinks(frontmatter)
          ...
          meta={
            <div className="mb-12 flex flex-wrap gap-4">
              <div className="group flex items-center gap-2 font-inter text-[13px] text-muted-foreground">
                <IconTag size={12} className="shrink-0 opacity-50 transition-transform duration-200 ease-soft group-hover:rotate-[-6deg] group-hover:scale-110" aria-hidden />
              ...
              <div className="mb-12 flex flex-wrap gap-3">
                {projectLinks.map((link) => (
                  <TrackedExternalLink
                    ...
                    projectSlug={slug}
                    projectTitle={displayTitle}
                    className={PROJECT_DETAIL_LINK_CLASS}
                  >
                    {PROJECT_DETAIL_LINK_ICONS[link.kind]}
                    {link.label}
                  </TrackedExternalLink>
                ))}
              </div>
